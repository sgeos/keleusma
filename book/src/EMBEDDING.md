...
The script must declare a structurally compatible type for the host's `Point` to flow correctly across the boundary. See [TYPE_SYSTEM.md](../../docs/spec/TYPE_SYSTEM.md) for the admissible interop types.
...
- `keleusma::audio_natives::register_audio_natives` registers `audio::midi_to_freq`, `audio::freq_to_midi`, `audio::db_to_linear`, `audio::linear_to_db`, and the `math::*` functions enumerated in [STANDARD_LIBRARY.md](../../docs/spec/STANDARD_LIBRARY.md).
...
The dialogue type, the yielded type and the resume type, must remain stable across swaps. The data segment may carry forward (pass current values), may be re-initialized to the new schema, or may be replaced by host migration code. Native function registrations live on the VM, not on the module, and persist across swaps. See [EXECUTION_MODEL.md](../../docs/architecture/EXECUTION_MODEL.md) for the full hot-swap specification, and [`examples/piano_roll.rs`](../../examples/piano_roll.rs) for a runnable end-to-end demonstration.
...
See `R42` in [`docs/decisions/RESOLVED.md`](../../docs/decisions/RESOLVED.md) for the design rationale and [`docs/spec/WIRE_FORMAT.md`](../../docs/spec/WIRE_FORMAT.md) for the header layout.
...
- [LANGUAGE_DESIGN.md](../../docs/architecture/LANGUAGE_DESIGN.md) describes the language model.
- [EXECUTION_MODEL.md](../../docs/architecture/EXECUTION_MODEL.md) describes the runtime model.
