...
The full list, signatures, and per-native contracts are in [`STANDARD_LIBRARY.md`](../../docs/spec/STANDARD_LIBRARY.md). The bundle's capability assessment and its standing limitations are in [`SHELL_AUDIT.md`](./SHELL_AUDIT.md).
...
The manifest declares a scheduler tick interval, a `[[task]]` table for each script with a name, a bytecode path, and a restart policy of `never`, `on_error`, or `always`, and an optional event table. The manifest format, validation rules, and scheduler semantics are documented in [`RUN_TASKS.md`](../../docs/architecture/RUN_TASKS.md).
...
- [`STANDARD_LIBRARY.md`](../../docs/spec/STANDARD_LIBRARY.md), [`RUN_TASKS.md`](../../docs/architecture/RUN_TASKS.md)
