...
- Approximately 1124 keleusma lib tests plus 364 integration tests across 29 files (including 53 rogue-script, 40 marshall, the 96-test multi-word fixed-point suite, the 18-test const-generics suite, the struct-field-index and generic-methods suites, and the flat-composite, persistent-data, narrow-word VM, and zero-copy suites), 42 keleusma-arena, and 6 keleusma-bench tests across the workspace covering lexer, parser, type checker, monomorphizer, compiler, VM, verifier, marshall, flat-byte composites, multi-word fixed-point arithmetic, const generics, trait methods on generic types, operator families (bitwise, shift, boolean), arena, audio natives, utility natives, target descriptor, visitor pattern, signing, IFC labels, cost-model calibration, and integration tests.
