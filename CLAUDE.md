...
**Status**: V0.2.0 published. V0.2.0 adds cryptographic module signing (Ed25519), the V0.2.0 ISA reset (69 opcodes, fixed-size opcode records, separately addressed operand pool, section-partitioned body), information-flow labels including negative labels, calibrated WCET cost models via the new `keleusma-bench` crate, and a docs/spec/ reorganization that consolidates authoritative specifications. V0.1.x retired surface features (closures, f-strings, `text` bundled DSL) are gone from V0.2.0; programs that used them must be rewritten under host-registered natives. The runtime is at `BYTECODE_VERSION = 1` (reset for V0.2.0; V0.1.x bytecode does not load). Approximately 826 lib tests plus 53 rogue-script + 37 arena + 17 marshall + 17 zero-copy + 6 bench across the workspace, all passing under default features, default+signatures, and `--all-features`. Hindley-Milner inference, generics with traits and bounds, compile-time monomorphization, target descriptor for cross-architecture portability, hot code swap, and the conservative-verification stance remain in place.
...
- Approximately 826 lib tests plus 53 rogue-script + 37 arena + 17 marshall + 17 zero-copy + 6 bench across the workspace covering lexer, parser, type checker, monomorphizer, compiler, VM, verifier, marshall, arena, audio natives, utility natives, target descriptor, visitor pattern, signing, IFC labels, cost-model calibration, and integration tests.
