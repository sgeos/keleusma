...
- Approximately 1149 keleusma lib tests plus 266 integration tests across 23 files (including 53 rogue-script, 31 marshall, the 53-test multi-word fixed-point suite, and the flat-composite, persistent-data, narrow-word VM, and zero-copy suites), 42 keleusma-arena, and 6 keleusma-bench tests across the workspace covering lexer, parser, type checker, monomorphizer, compiler, VM, verifier, marshall, flat-byte composites, multi-word fixed-point arithmetic, arena, audio natives, utility natives, target descriptor, visitor pattern, signing, IFC labels, cost-model calibration, and integration tests.
