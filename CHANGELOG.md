...
- **`Module::shared_data_bytes` (`u32`) and `Module::private_data_bytes` (`u32`).** Verifier-populated byte counts for the shared and private partitions of the data segment. Mirrored in the framing header at offsets 24 and 28. Shared data lives in the Vm's slot storage and is host-visible through `Vm::set_data` and `Vm::get_data` as today. Private data lives in the arena's persistent (`.data`) region added in `keleusma-arena` 0.3.0 and is not exposed through the host API. Both byte counts are zero in this release because the language surface for `shared` and `private` keywords lands in a subsequent phase; the fields are wired through the wire format now so producers and consumers do not need a second round of header changes.
