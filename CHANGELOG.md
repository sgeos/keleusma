...
- **`TextSize` lattice for WCMU text-size tracking.** New `keleusma::text_size` module introduces the abstract-interpretation lattice (`TextSize::{Known(u32), Unbounded}`) and the `op_cost_context` helper that lifts a pair of lattice values into the `OpCostContext` consumed by `OpCost::Dynamic` cost evaluators. The lattice supports saturating addition and the join (maximum) operation. Integration with `verify::compute_chunk_wcmu` so that programs whose text-producing opcodes exceed the arena's top region are rejected at the safe constructor is staged for V0.2.x. Until the integration lands, the runtime exhaustion path through `VmError::OutOfArena` continues to provide graceful failure for programs that allocate beyond the arena.
