...
- **`shell` cargo feature, default off**. New cargo feature that compiles `stddsl::Shell`. Requires `std::env` and `std::process::Command`; therefore incompatible with `no_std` builds. The `keleusma-cli` crate enables the feature so the CLI runner has shell access. Shell functions: `shell::getenv(name: Text) -> Text` (empty string for unset; companion `shell::has_env(name: Text) -> bool` for presence checking), `shell::run(cmd: Text) -> (Word, Text)` (executes through `sh -c`, returns exit code and stdout), `shell::run_checked(cmd: Text) -> Text` (returns stdout, traps on non-zero exit), `shell::exit(code: Word)` (terminates the host process). The originally specified `getenv -> Option<Text>` shape was infeasible because the compiler's `Option::Some(x) =>` pattern emits an `IsEnum` check that fails against the runtime convention that `Some(v)` is `v` directly; the shell-idiomatic empty-string shape is the practical alternative.
