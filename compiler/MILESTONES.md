...
| 1 | **Codegen** (`kel/codegen.kel`) | The largest stage, migrated inside-out over several increments: the emit-to-host boundary plus a logical-module round-trip first, then compile, then monomorphization, then per-declaration typecheck. Yields the ops the host assembles into the module. **Increments 0 through 2 landed** (`tests/selfhost_codegen.rs`). Increment 0 proved the emit boundary with a trivial constant. Increment 1 transcribed `input * 2 + 1`, proving the op vocabulary a real emitter needs, the parameter load, a two-entry constant pool, and the checked-arithmetic-then-`PopN` lowering. Increment 2 is the first **real codegen driven by input**: the first throwaway adapter derives a declaration's single-node body from the Rust reference and hands it to the stage through the `decl` shared-data block, and the stage computes the ops, dispatching a literal body to `Const` and a parameter body to `GetLocal`, then wrapping with `Return`. All are validated by structural equality against the Rust compiler and by running the built module. The module frame, constant pool, and parameter layout are still host-supplied, and bodies are single-node; nested expressions, which need the work-stack idiom, and emitting the frame and pool from Keleusma come next. | Rust lex, parse, and initially typecheck and monomorphization, mapped into the Keleusma declaration stream. The adapter delivers progressively-less-processed declarations as the inner increments advance. |
