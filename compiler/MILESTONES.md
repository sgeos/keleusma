...
| **V0.2.x** | **Step 1 — lexer** | `kel/lexer.kel`: a `loop` byte scanner with a keyword table, yielding `Token`s. Wired into the Rust pipeline behind `--lexer keleusma`. **Increment 1 landed**: a streaming byte tokenizer for whitespace, identifiers, integers, and single-byte punctuation, yielding the transitional `kind + value*16` token wire; it compiles, verifies, and runs on the current runtime (`keleusma-selfhost lex <file>` and `tests/selfhost_lexer.rs`). Remaining increments: spans, string interning and keyword resolution via the `compiler::` natives, multi-byte operators, string and float literals, the richer `Token` struct yield, and the corpus-validation gate. | Every program in the regression corpus compiles to **byte-identical** bytecode under Keleusma-lexer + Rust-parser + Rust-compiler as under the all-Rust baseline. |
