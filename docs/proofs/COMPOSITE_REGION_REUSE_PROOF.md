...
> verifies. That scoping has narrowed: M6(d) is reported structurally enforced by `verify()` as
> of 2026-08-23, commit pending record in Appendix E, after which the sole remaining emission
> invariant is M5's stream-never-returns half. Theorem B2 is additionally a **proved specification, not a description**. The
...
2. **A standing guarantee for M5's second half, and formerly for M6(d).** The V0.2.X line
   reports M6(d) structurally enforced by `verify()` as of 2026-08-23, commit pending record, so
   the remaining emission invariant is that streams never return, which closes M5's
   nested-stream hole and re-runs as a pin on every build of that line. Until the enforcing
   commit is recorded, and always for M5, a future returning stream or hand-written bytecode
   defeats the reuse theorems without failing verification.
...
| verifier floors operand-stack pops at loop entry, converting M6(d) from an emitted invariant into an enforced one | `src/verify_typed.rs` | V0.2.X line | **decided and implemented on that line, reported 2026-08-23**, at zero measured rejections, with the empty-entry coincidence of the M6 row explaining the zero. The commit is to be recorded in Appendix E |
...
derivation was confirmed against `apply_op`'s underflow guard, and the V0.2.X line reports that
`verify()` now floors every scope explicitly, so M6(d) is structurally enforced and the change
cost zero rejections for the reason the derivation gives. The enforcing commit and the
disposition of the `tests/loop_entry_floor.rs` gap pins, which were built to fail when the gap
closed, are to be recorded here when that line sends them. The source-form enumeration landed
