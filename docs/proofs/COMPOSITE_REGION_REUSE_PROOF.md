...
> instantiation status is a proved specification only. Pending external input, marked in rows,
> one gate commit from the V0.2.X line carrying the M8 discriminator pin, the `SetDataIndexed`
> pin, and the Break row correction. A targeted re-audit of the sections this revision changed
...
3. **The M8 discriminator's pin**, pending in the V0.2.X gate commit.
