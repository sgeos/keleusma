...
The shift operators `<<`, `<<<`, `>>`, and `>>>` apply to a `Word` or a `Multiword<N, F>` value. The shift amount is a `Word`, and in the current increment it must be a compile-time-constant non-negative integer within the value's total bit width; a variable amount is a later increment, and an out-of-range amount is rejected at compile time. The `Byte` type is not yet shiftable.

The four operators follow the Verilog convention, so the plain `<<` and `>>` are the logical shifts and the tripled `<<<` and `>>>` are the arithmetic ones. This convention was chosen over the Java one because the mission-critical language cluster the language targets makes the arithmetic-versus-logical distinction explicit rather than inferring it from operand signedness, and because it lets the arithmetic left shift carry overflow semantics that the logical left shift does not.

| Operator | Direction | Fill | Meaning |
|----------|-----------|------|---------|
| `<<` | left | zero at the bottom | logical left, wrapping and total |
| `<<<` | left | zero at the bottom | arithmetic left, the value `x * 2^k`, admits overflow capture |
| `>>` | right | zero at the top | logical right, unsigned divide, total |
| `>>>` | right | sign at the top | arithmetic right, floor divide, total |

A left shift vacates the low bits and fills them with zero regardless of arithmetic or logical interpretation, so `<<` and `<<<` produce the identical value. Their only difference is that `<<<` is the value `x * 2^k` and therefore admits the `overflow` and `underflow` arms of the checked-arithmetic construct, while `<<` is a plain wrapping bit operation with no overflow concept. The two right shifts differ only in the fill and coincide for a non-negative value. The right shifts and the logical left shift never overflow. Shifts bind below the comparisons and above the additive operators.
