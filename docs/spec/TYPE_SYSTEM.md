...
The three operators follow the Java and JavaScript convention. `<<` is the left shift, which zero-fills the vacated low bits and drops the bits shifted out of the top, so it wraps like the other arithmetic. `>>` is the arithmetic right shift, which fills the vacated high bits with copies of the sign bit, so it preserves the sign. `>>>` is the logical right shift, which fills the vacated high bits with zero. The two right shifts coincide for a non-negative value and differ only on the sign bit. Shifts bind below the comparisons and above the additive operators.
