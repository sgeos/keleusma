...
The compiler emits the field when invoked with debug enabled (`compiler::compile_with_options` with `emit_debug`, surfaced as `keleusma compile --debug`); the `keleusma strip` subcommand removes it. The compiler emits `CallSite`, `SourceSpan`, `LineNumber`, `VariableName`, `OptimisationMarker`, `GenericInstantiation`, `IfcLabelAnnotation`, `TypeAnnotation`, `AssertionContext`, and `BreakpointCandidate` records, and the verifier stage appends `WcetMarker` (per Stream chunk) and `VerifierWitness` records. A `VerifierWitness` is emitted once per discharged verification obligation, with operands `[pass, property]` and the `op_index` of the construct it concerns: the structural passes (`block-nesting-and-offsets`, `block-type-constraints`, `productive-divergence`) contribute one obligation per construct, and the `resource-bounds` pass contributes the per-iteration WCET and WCMU bound proofs for each Stream chunk. The witness scope is per-iteration; admission against a host arena capacity is a separate load-time check and is not asserted by the witness. All twelve catalogue kinds now emit. The field was added within the V0.2.x line without a `BYTECODE_VERSION` bump, consistent with the project's no-production-traction stance; a runtime built before B29 does not know the optional section.
