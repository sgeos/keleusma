...
| [`examples/rogue/`](../../examples/rogue) | End-to-end SDL3 video host driving a roguelike. Fourteen Keleusma scripts for dungeon generation, player and monster artificial intelligence, combat resolution, and item effects. Feature-gated. Run with `cargo run --release --example rogue --features sdl3-example,text`. See [ROGUE.md](./ROGUE.md) for the manual |
