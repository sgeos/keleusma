...
**Dual-end arena** -- The arena's stack-and-heap arrangement, in which two bump pointers grow from opposite ends of a single contiguous buffer. There is no fixed boundary. Allocation fails only when the two pointers would meet. The verifier proves at compile time that worst-case stack consumption plus worst-case heap consumption stays under the arena size. Implemented as the `Arena` type in the standalone `keleusma-arena` crate (re-exported as `keleusma::Arena`), with `BottomHandle` and `TopHandle` (aliased `stack_handle` and `heap_handle`) exposing the `allocator_api2::Allocator` trait. A configurable persistent prefix region survives every reset and backs the `private data` segment.

**BottomHandle / TopHandle** -- Allocation handles for the two ends of an `Arena`. Both implement `allocator_api2::Allocator`. Passed to `allocator_api2::vec::Vec::new_in(handle)` and similar constructors to obtain arena-backed collections. The two-handle design distinguishes the two arena ends at the type level rather than through a runtime discriminator. Defined in the standalone `keleusma-arena` crate. The keleusma runtime re-exports them with the legacy names `StackHandle` and `HeapHandle` for compatibility.

**Budget** -- A producer-agnostic memory budget structure defined in `keleusma-arena`. Holds `bottom_bytes` and `top_bytes` worst-case bounds. The arena's `fits_budget` method checks admissibility against the arena's capacity. Independent users compute budgets through profiling, manual analysis, or any other mechanism. The Keleusma runtime computes its budget from static analysis of bytecode through `verify::budget_for_stream`.

**BottomMark / TopMark** -- Snapshot types for the LIFO mark and rewind discipline on the two arena ends. Returned by safe `Arena::bottom_mark()` and `Arena::top_mark()`. Passed to unsafe `Arena::rewind_bottom` and `Arena::rewind_top` to restore an end to a previous mark.
...
**Cost table** -- A mapping from each bytecode instruction to its execution cost, implemented as `Op::cost()` in `src/bytecode.rs`, which delegates to `NOMINAL_COST_MODEL.cycles(self)`. The nominal `CostModel` assigns each opcode a pipelined-cycle weight; the `keleusma-bench` crate calibrates a per-host `MEASURED_COST_MODEL` from measured cycle counts. Used by `wcet_stream_iteration()` (and its `_with_cost_model` variant) for worst-case execution time analysis. WCET is reported in pipelined cycles; a host applies a platform-specific factor to convert to wall-clock time. These weights are order-of-magnitude correct and subject to per-host calibration.

**Data segment** -- The fourth memory region in the Keleusma runtime, corresponding to the conventional `.data` section of an executable. Mutable storage presented to the script as a preinitialized context, declared in source through a `data` block. It comes in two categories. A bare `data` block is *shared* data: a host-owned, borrowed `&mut [u8]` buffer lent to the VM per run via `Vm::call_with_shared`/`resume_with_shared` and marshalled through `Vm::get_shared`/`set_shared`. A `private data` block resides in the arena's persistent region and persists across yield and reset boundaries, including hot swap. Both are read and written by the script via the `GetData` and `SetData` instructions. Schema may change arbitrarily across hot updates. Conceptually analogous to the persistent state of an Open Telecom Platform `gen_server` [H1, H2] and to the state vector of a SCADE mode automaton [H3, SC1]. See [EXECUTION_MODEL.md](../architecture/EXECUTION_MODEL.md) and [RELATED_WORK.md](./RELATED_WORK.md) Section 8.
...
**replace_module** -- The Vm method that performs a hot code update. The host calls it between a `VmState::Reset` and the next `call`. The new module is verified before replacement. The host supplies an initial private data segment instance whose length must match the new module's declared private slot count; shared data is the host-owned `&mut [u8]` buffer supplied at the next `call_with_shared` and persists across the swap in the host's own buffer rather than being passed as a hot-swap input. Frames and stack are cleared so the next `call` starts the new module's entry point. The same mechanism supports forward update and rollback. Dialogue type compatibility across the swap is the host's responsibility because dialogue types are erased at the bytecode level.
...
**Value** -- The runtime representation of data in the VM. An enum with variants: Unit, Bool, Int, Byte, Float, StaticStr, KStr, Tuple, Array, Struct, Enum, None, and Opaque.
