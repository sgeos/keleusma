...
- Structural verifier: block nesting and offset validation, block-type constraints, and
  ...
  *Slice 1 landed (`compiler/kel/verify_structural.kel`, driver `selfhost::structural_reject_*_via_kel`, gate `compiler/tests/verify_structural.rs`).* It reproduces the block-nesting and branch-target-bounds subset decidable from the marshalled `(class, arg)` op table alone: `If`/`Loop` exit-target bounds, `Else` target bounds, `Else`/`EndIf`/`EndLoop` block matching, empty-stack closure, and `Break`/`BreakIf` loop containment. It is a sound necessary condition (every chunk it rejects the reference also rejects), gated positively against the five stage sources plus an ephemeral program and negatively against mutated bytecode. *Deferred slices*: (a) the exact target-equality checks (`If`-without-`Else` targets its `EndIf`, `Else` targets its `EndIf`, `Loop` exit equals `EndLoop + 1`, `Break` target equals the loop exit, `EndLoop` back-edge equals `Loop + 1`) — these require extending `analyze_class` to carry the `Break`/`BreakIf`/`EndLoop` targets it currently drops to zero, plus forward marker-position lookup; (b) local-slot, data-slot, constant-pool, call-target/arity, fraction-bit, and template-index bounds; (c) block-type constraints; (d) productive-divergence and yield-coverage.
