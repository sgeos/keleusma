...
**Status**: Implementation under way in [`compiler/`](../../compiler/README.md); see [`compiler/MILESTONES.md`](../../compiler/MILESTONES.md) for the running log. All three stages exist in Keleusma source and each **self-compiles byte-identically** to the Rust-hosted compiler (`lexer.kel`, `codegen.kel`, `parse.kel`), driven end to end by the `keleusma-selfhost compile` command and pinned by `compiler/tests/fixed_point.rs`. Two gaps remain before the success criteria below are met: the postorder-record-to-forest reconstruction between the parser and code-generator stages is still host-side Rust rather than a Keleusma stage, and the emitted module borrows its data layout, constant-pool metadata, and auxiliary body from the Rust-hosted reference rather than assembling them from the stage output. The research pass (below) remains the authoritative design; the note that follows records only that the schedule has advanced from "not started" to "three stages self-compiling."
