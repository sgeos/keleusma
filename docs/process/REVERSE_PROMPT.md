...
**Date**: 2026-06-04
**Status**: B28 P2 is complete and merged-and-pushed on `feat-flat-memory-model` (nested inlining, the layout fold, and arena residence Phase 1 and Phase 2; the P2 row in `BACKLOG.md` is marked complete). P4 (precise WCMU) is in progress on the sub-feature branch `feat-flat-memory-wcmu`: groundwork landed (commit `ca419bb`); the main ISA consolidation is designed and staged below, not yet built. Gate green through `ca419bb`. Committed locally, not pushed.
...
- Done (`ca419bb`): the four constructors delegate to one `GenericValue::try_pack_flat` packer that appends fields and zeros only the slack; `min_bytes` is the explicit allocation. Behaviour-preserving, gate green.
- Remaining (the opcode consolidation, the large part): define `Op::NewComposite` and its operand (inline flat / pool boxed); add it to every exhaustive `Op` match (bytecode `cost`/`stack_growth`/`stack_shrink`, wire `encode_op`/`decode_op`/id table/size table, verify `op_depth_effect`/WCMU/const-pool validation, the three bench cost models, the VM handler dispatching by kind to the existing `*_with_widths`); make the WCMU pass sum the `byte_size` operand; have the compiler emit `NewComposite` with the baked `byte_size` at every construct site; then remove `NewStruct`/`NewTuple`/`NewArray`/`NewEnum` and update golden bytecode and the tests that name them. Build green at each milestone (add coexisting, migrate emit, then remove old).
