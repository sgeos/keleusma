...
1. **Text epoch sourcing (safety).** Extraction of a flat `Text` field currently rebuilds the `KString` against the *current* arena epoch. The host-boundary decode of a materialised composite held across a `RESET` therefore reattaches the wrong epoch and dereferences reclaimed memory. Fix: reattach the **originating composite's** epoch so the epoch check governs the read. The materialised (inline) form must retain that epoch so the reattachment is correct after materialisation. Pin the regression with a yield/return → `RESET` → decode test that must observe a clean `Stale` outcome, not undefined behaviour.
2. **Transitive string flow restrictions (type checker).** Extend the cross-yield prohibition and data-segment exclusion to descend into composite field and variant-payload types, so a composite transitively containing `Text` is treated exactly like a bare dynamic string. This closes the undetected-yield path that the current value-walking `contains_dynstr` check misses for flat bodies.
