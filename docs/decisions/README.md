...
| [STREAM_SPILL_LAYOUT.md](./STREAM_SPILL_LAYOUT.md) | **Open, framed not taken**: the two remaining resumable-stream refusals are one question — operand entries live in SSA values and do not survive a return — needing a statically sized ephemeral slice; the multi-parameter refusal is separated out, because that shape FAULTS on the reference |
...
| [CONFINEMENT_CONSUMPTION_BRIEF.md](./CONFINEMENT_CONSUMPTION_BRIEF.md) | Consuming the confinement verdict to REFUSE and only to refuse, which buys the soundness half of the region-planner obligation without the exposure half that overlapping sites would take on |
| [FLOAT_LADDER.md](./FLOAT_LADDER.md) | **Ruled 2026-08-31**: `f16` is IEEE `binary16` and `f8` is OFP8 `E5M2`, chosen for one semantic model at every rung and a shared exponent field; widen-compute-narrow; `Fixed<8>` stays the default for control work |
| [TEXT_N_REQUIREMENTS.md](./TEXT_N_REQUIREMENTS.md) | **Addressed to the `v0.2.3` line, and a RELAY**: the operator's `Text<N>`, static-`Text`, and `Opaque`-address-sizing rulings, with the requirements the V0.3.X backend depends on and the evidence for each |
