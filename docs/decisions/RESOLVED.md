...
Compiled Keleusma modules can be serialized to a self-describing byte form and loaded back at runtime through `Module::to_bytes` and `Module::from_bytes`. The wire format is the four-byte magic `KELE` followed by a little-endian sixteen-bit version followed by the postcard-encoded module body. The header allows the runtime to reject foreign or incompatible bytecode at load time before any deserialization is attempted. Bytecode version 1 is paired with `BYTECODE_VERSION = 1` in the crate. A change to any serialized type bumps the version.
