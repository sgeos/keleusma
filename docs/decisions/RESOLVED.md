...
The current `Module::from_bytes` path (path A) copies the body bytes into an `rkyv::util::AlignedVec<8>` before calling `rkyv::from_bytes`. The copy ensures alignment regardless of the host slice's alignment. Path B (zero-copy) is admissible as a follow-on but requires the host to supply an aligned buffer and adds a lifetime parameter to the `Vm`.
