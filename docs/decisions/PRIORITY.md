...
The bounded-memory guarantee now holds end to end. Native-produced dynamic strings can flow through `Value::KStr` with epoch-tagged stale-pointer detection. The legacy `Value::DynStr(String)` path remains for natives that do not need arena allocation.
