...
9. ~~Native ABI extension with arena context.~~ Complete. The `NativeCtx<'a>` carries a borrow of the host-owned arena and is created freshly at every `Op::CallNative` dispatch. The new `Vm::register_native_with_ctx` and `Vm::register_native_with_ctx_closure` register natives that receive the context. The marshall layer's `IntoNativeFn` and `IntoFallibleNativeFn` traits accept the context internally and ignore it for marshalled functions that do not need it. The utility natives module exposes `register_utility_natives_with_ctx`, which registers `to_string` to produce `Value::KStr` and `length` to resolve `Value::KStr` arguments through the arena. The legacy `register_utility_natives` remains for hosts that prefer `Value::DynStr` outputs.

The bounded-memory guarantee now holds end to end. Native-produced dynamic strings flow through `Value::KStr` with epoch-tagged stale-pointer detection. (V0.2.0 update: the legacy `Value::DynStr(String)` global-heap path was removed; all dynamic strings are arena-resident `Value::KStr`. The `register_utility_natives_with_ctx` alias was deprecated alongside the removal of the `to_string`, `concat`, `slice`, and `length` bundled natives. Dynamic-text composition is the host's responsibility through `register_verified_native` or `register_fn`.)
...
2. ~~Enforce the fixed-size field type constraint at the data block declaration boundary, per the table in [TYPE_SYSTEM.md](../spec/TYPE_SYSTEM.md).~~ Complete.
