...
| H2 | Med | **OPEN, not fixed** | The recursive-descent **parser** overflows the stack and aborts on deeply nested source, independently of any bytecode. Measured on a two-mebibyte stack: `fn main() -> Word {{ let a = 7; if a > 0 {{ ... }} else {{ 0 }} }}` nested **20** deep compiles and verifies; at **22** the process aborts inside `parse`, between lexing and compilation. This is a compile-time denial of service on untrusted **source**, which the command-line front end, the language server and the playground all accept. It is a different component and a different fix from H1 and was not made here rather than expanding an increment; it is recorded with its measurement so it is not rediscovered. Its severity depends on whether a host compiles source it did not write, which the shipping binary does. |
