...
Phase 5 status detail. The shift operators are a language-wide surface addition, not a Multiword-internal one, because Keleusma had no shift or bitwise operators at all before this phase. Four operators are added, following the Verilog convention: `<<` logical left shift, `<<<` arithmetic left shift, `>>` logical (zero-fill) right shift, and `>>>` arithmetic (sign-preserving) right shift. They apply to a `Word` or a `Multiword<N, F>` value shifted by a `Word` amount, and bind below the comparisons and above the additive operators. The amount is a compile-time constant within the value's total bit width in this increment; a variable amount is a stretch item, and `Byte` shifts are not yet supported.

The Verilog convention was adopted over the Java one after a comparative review, because the mission-critical language cluster the Justification compares against makes the arithmetic-versus-logical distinction explicit rather than inferring it from operand signedness, and because a left shift is a single operation on the data, so a plain `<<` and `<<<` pair is redundant unless the arithmetic one is given a distinct meaning. That meaning is overflow capture: `<<<` is the value `x * 2^k`, which can exceed the range, so on the word-width type it admits the `overflow` and `underflow` arms of the checked-arithmetic construct, lowering to a checked multiply by the constant `2^k`, while `<<` is a plain wrapping bit operation. The right shifts and the logical left shift never overflow. On the multi-word type, which has no checked construct, `<<<` wraps identically to `<<`. The overflow capture reuses the existing checked-multiply machinery in full, so no new opcode or fault path is added; the amount for a checked `<<<` is bounded to `word_bits - 2` because the multiplier `2^k` must be a positive Word. The capture is verified by tests for the `ok` outcome, the `overflow` and `underflow` outcomes, the low-half binding of the wrapped result, and the `saturate_max` resolution in the overflow arm.

The one hard implementation problem was the `>>` versus nested-generics conflict: a type such as `Option<Option<Word>>` lexes its stacked close to a single `>>` (or `>>>`) token, which would break generic parsing. The resolution is the standard Rust and Java one, token splitting at the generic close: the lexer emits `<<`, `>>`, and `>>>` greedily, and a `gt_debt` counter on the parser lets a generic close consume one `>` from a `>>`/`>>>` token and record the remainder for the immediately-following stacked closes. A regression test confirms `Option<Option<Word>>` still parses.
