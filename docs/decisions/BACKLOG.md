...
> **Implementation status (V0.2.1): framework landed; catalogue and VM wiring partial.** The chunk-local `debug_pool` section, its data model, and its canonical byte encoding are implemented ([`src/debug_meta.rs`](../../src/debug_meta.rs)), carried through the wire format as an optional per-chunk field that leaves the opcode stream byte-identical ([`src/wire_format.rs`](../../src/wire_format.rs), documented in [`WIRE_FORMAT.md`](../spec/WIRE_FORMAT.md)), emitted by the compiler under `compile_with_options` / `keleusma compile --debug` (currently the `CallSite` record only), removed by the `keleusma strip` subcommand, and read back through a query API (`DebugPool::records_at`, `source_location`). Invariants 4 and 5 are exercised end to end: a `--debug` build strips to byte-identical release bytes. Carried forward: the remaining record kinds in the catalogue below (same encoding as `CallSite`), and wiring the read path into the VM trap path, which needs a fault location (chunk and op index) threaded through the executor and `VmError`.
