...
>
> The debug items still carried forward, now that the catalogue emits in full: the finer **function-entry and operator** `BreakpointCandidate` granularities (above); a finer per-construct `VerifierWitness` trace produced inline by the verify pass (the current witness is a per-chunk admission summary); and wiring the read path into the VM *trap* path. On the last: `BreakpointHit` now exposes a `(chunk, op)` position at a suspension point, which is the same shape a trap would need, but `VmError` still carries no fault location, so a runtime trap (an unhandled partial operation, or a debug `assert`) cannot yet be mapped back to its source through the records the compiler emits. This last item is the only remaining cross-cutting enabler.
