...
>
> Emitted today: `CallSite` (per call instruction), `SourceSpan` and `LineNumber` (per statement), `VariableName` (per declared local slot), `OptimisationMarker` (at each refinement-elision site, where the compiler proves a refinement predicate at compile time and omits the runtime check), `GenericInstantiation` (per monomorphized chunk, naming the generic origin and canonical type arguments, threaded from the monomorphizer through `monomorphize_with_provenance`), and `IfcLabelAnnotation` (at each `classify`/`declassify` operation, recording the label set for the information-flow audit trail). The remaining four kinds are defined, encodable, decodable, and (for span kinds) resolvable, but are not yet emitted because their data is not available to the compiler backend: `TypeAnnotation` needs a `TypeRepr` encoding and a per-op operand-stack type model; `WcetMarker` is a verifier-stage product computed after compilation; `AssertionContext` needs a source `assert` construct, which the language does not have; `VerifierWitness` needs a structured verifier-acceptance trace that is not kept. Also carried forward: wiring the read path into the VM trap path, which needs a fault location (chunk and op index) threaded through the executor and `VmError`.
