...
>
> **The B29 record catalogue is complete: all twelve kinds emit.** The breakpoint *runtime mechanism* is now implemented: the VM holds a `breakpoint_positions` list, suspends with `GenericVmState::BreakpointHit { chunk, op }` before executing an armed op (gated on an `is_empty` fast path so unarmed programs pay nothing), exposes `set_breakpoint`/`clear_breakpoint`/`clear_breakpoints`/`breakpoint_count` and `resume_from_breakpoint` (a value-free resume with one-shot re-trigger suppression), and arms a position from a `BreakpointCandidate` record's op index. Breakpoints are runtime state, not bytecode, so they do not affect the verified WCET or WCMU bounds. Breakpoint candidates now emit at statement boundaries, block tail expressions, and function entry (op 0); the finer **operator granularity** (a candidate at every operator op) remains future work, since an every-op candidate set is high-volume and of marginal value over the boundary candidates.
>
> The read path into the VM *trap* path is now wired. The VM records the op it is dispatching in a `fault_location` field (a single `Option` write per op, cleared by the `run` wrapper on success), so after a failed `call`/`resume` it names the faulting op. `Vm::fault_location() -> Option<(chunk, op)>` exposes it, and `Vm::fault_source_location() -> Option<FaultSource>` maps it back to source by decoding the faulting chunk's debug pool on demand: tier 1 returns a span-bearing record (`CallSite`, `SourceSpan`, `AssertionContext`, `BreakpointCandidate`) sitting exactly at the fault op (`exact = true`); tier 2 falls back to the nearest enclosing statement's `SourceSpan` (`exact = false`). It never fabricates a location, and the owned `FaultSource` carries the `exact` flag so a host does not over-trust a fallback. A failed debug `assert` resolves exactly through its `AssertionContext`. The compiler now also emits a `SourceSpan` for each block's tail expression, so a fault in a function whose body is one expression resolves to that expression's span rather than to nothing; a mid-statement partial-operation trap still resolves to the tightest enclosing statement or expression (`exact = false`) rather than the precise sub-expression, which would need per-op source spans.
>
> The debug items still carried forward have narrowed to three precision refinements, each a tightening of an already-faithful result rather than a coverage gap: (1) the **operator `BreakpointCandidate` granularity** (statement-boundary, tail-expression, and function-entry candidates emit; an every-operator candidate set remains future work as high-volume and marginal); (2) a finer **per-segment Reentrant WCET** (the witness now records a Reentrant chunk's whole-body cumulative WCET, a sound but loose upper bound on any single resume; a per-segment analysis costing execution between consecutive yields would tighten it); and (3) the finer **per-op `SourceSpan` granularity** that would let a mid-statement fault resolve to the exact sub-expression rather than the tightest enclosing statement or tail expression.
