...
| P5 | Hot-code-swap migration over flat bytes, documentation (`EXECUTION_MODEL.md`, the dual-end-arena and hot-swap decisions; the `NewComposite` consolidation is already reflected in `INSTRUCTION_SET.md` and `WIRE_FORMAT.md` from P4), B26 and B27 marked resolved through B28, and B28 closure. | Pending |
...
## B36. Narrow-word composite width: `from_value` versus `from_value_ctx` contract (Open)
...
**Decision needed.** Either (a) declare `from_value` a bundled-runtime convenience and require `from_value_ctx` (module widths) for all composite decoding on narrow-word builds, then move native-result production to module widths and update the audio test helpers and any other `from_value` composite decoders; or (b) keep native results at runtime widths and address narrow-word script field-access of native composite results another way; or (c) document that composite values do not cross the native boundary by-value on narrow-word builds. Option (a) is the most internally consistent (one canonical module-width layout for every composite, decoded through the context-aware path) but widens the blast radius of the narrow-word feature and should be weighed against how much narrow-word composite-at-the-boundary use is actually intended. No code is blocked on this; it is a correctness-completeness item for the narrow-word portability feature.
