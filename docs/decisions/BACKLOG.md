...
| 3 | Bound the compiler front end against pathological input, a denial-of-service surface distinct from runtime WCET. The parser already caps recursion at `MAX_PARSE_DEPTH = 24`; confirm or bound the type checker and monomorphizer against inference and instantiation blow-up. | partial | medium | `src/parser.rs`, `src/typecheck.rs`, `src/monomorphize.rs` |
