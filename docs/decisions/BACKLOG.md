...
>
> Emitted today (all twelve): `CallSite` (per call instruction), `SourceSpan` and `LineNumber` (per statement), `VariableName` (per declared local slot), `OptimisationMarker` (at each refinement-elision site, where the compiler proves a refinement predicate at compile time and omits the runtime check), `GenericInstantiation` (per monomorphized chunk, naming the generic origin and canonical type arguments, threaded from the monomorphizer through `monomorphize_with_provenance`), `IfcLabelAnnotation` (at each `classify`/`declassify` operation, recording the label set for the information-flow audit trail), `TypeAnnotation` (per declared local slot, carrying a string-form `TypeRepr` v1 in the type sub-pool), and `WcetMarker` (per Stream chunk, the verifier-stage per-iteration WCET appended to the built `debug_pool`, carried as two `u16` operands reconstructing the `u32` cycle count). The `TypeAnnotation` scope is the per-local declared or inferred type, not the full per-op operand-stack type model that finer-grained stack introspection would require; the latter remains future work. `AssertionContext` emits at each debug `assert`: a compile-out `assert` construct was added to the language (grammar, parser, type checker, codegen, and the `AssertionFailed` trap), and under a debug build each `assert` emits an `AssertionContext` record carrying the source span and optional message. `assert` is a contextual keyword like `classify`/`declassify`; the check compiles out entirely in a non-debug build, so a debug and a release build are distinct compilations rather than one artefact bridged by `strip`. See [`GRAMMAR.md`](../spec/GRAMMAR.md) and [`RUNTIME_FAULTS.md`](../spec/RUNTIME_FAULTS.md). `BreakpointCandidate` emits at each statement boundary (the default granularity), recording the op position and source span a debugger reads to present breakpoint choices. `VerifierWitness` emits a per-construct verification trace at the verifier stage. `verify::chunk_verification_witness` returns the per-chunk admission summary (the passes that admitted the chunk), and `verify::chunk_verification_obligations` returns the finer **structural** trace: one `VerificationObligation` per property the three structural passes of `verify` establish, each keyed to the op position of the construct it concerns. Under a debug build the compile pipeline emits one `VerifierWitness` record per obligation, with operands `[pass, property]` and the construct's `op_index` (chunk-level facts such as `all-blocks-closed` carry `op_index = 0`). A reader groups them with `DebugPool::records_at`. Pass 1 records one obligation per block, offset, and data-slot construct (`if-branch-target-in-bounds`, `endloop-back-edge-targets-loop-entry`, `data-slot-in-range`, and so on); pass 2 records the block-type constraints keyed to the marker ops; pass 3 records `every-stream-to-reset-path-yields` at the Stream op.
>
> Beyond the structural passes, the compile pipeline emits two **resource-bound** obligations (pass `resource-bounds`) for each Stream chunk, on the arms where the bound is actually proven: `wcet-per-iteration-bound-proven` when `wcet_stream_iteration` returns a finite cycle count (the count itself is carried by the adjacent `WcetMarker`), and `wcmu-per-iteration-bound-proven` when `wcmu_stream_iteration` returns a finite total. These are the *per-iteration* bounds only. Admission of the program against a host's arena capacity is a separate load-time analysis (`verify_resource_bounds`) run at `Vm::new`, not at compile time, so the witness deliberately does not assert it. A chunk whose bound overflows records no resource-bound obligation, so absence is the faithful signal.
>
> The trace is faithful only because the compile pipeline produces the structural obligations after `verify` returns `Ok` and the resource-bound obligations on the proof arms themselves, so each obligation records a fact a successful verification guarantees rather than re-deriving it. It is a faithful projection of the verifier's checks, not a machine-checkable derivation, and some property identifiers stand in for more than one underlying check. Producing the structural obligations inline as the verify pass discharges them, rather than by re-walking afterward, remains a possible refinement.
>
> **The B29 record catalogue is complete: all twelve kinds emit.** The breakpoint *runtime mechanism* is now implemented: the VM holds a `breakpoint_positions` list, suspends with `GenericVmState::BreakpointHit { chunk, op }` before executing an armed op (gated on an `is_empty` fast path so unarmed programs pay nothing), exposes `set_breakpoint`/`clear_breakpoint`/`clear_breakpoints`/`breakpoint_count` and `resume_from_breakpoint` (a value-free resume with one-shot re-trigger suppression), and arms a position from a `BreakpointCandidate` record's op index. Breakpoints are runtime state, not bytecode, so they do not affect the verified WCET or WCMU bounds. Two breakpoint items remain: the finer **function-entry and operator granularities** for `BreakpointCandidate` emission (only the statement-boundary default exists), selectable through a compile-time flag.
>
> The debug items still carried forward, now that the catalogue emits in full and the `VerifierWitness` trace is per-construct: the finer **function-entry and operator** `BreakpointCandidate` granularities (above); the **inline** `VerifierWitness` refinement, producing obligations as the verify pass discharges them rather than by re-walking the chunk afterward (the emitted trace is already per-construct, so this is an internal refactor, not a coverage gap); and wiring the read path into the VM *trap* path. On the last: `BreakpointHit` now exposes a `(chunk, op)` position at a suspension point, which is the same shape a trap would need, but `VmError` still carries no fault location, so a runtime trap (an unhandled partial operation, or a debug `assert`) cannot yet be mapped back to its source through the records the compiler emits. This last item is the only remaining cross-cutting enabler.
