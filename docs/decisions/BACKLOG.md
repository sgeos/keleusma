...
- B29 (strippable debug opcodes) is independent of B28. The strippable opcode framework still applies to debug aids like variable names, source spans, breakpoints; it no longer carries a `DataSlotAnnotation` opcode load-bearing for B28 because the runtime computes layouts from existing wire-format metadata.
...
B29 proposes a strippable debug opcode framework: compiler-emitted opcodes that the runtime consumes for development aids (variable names, source spans, breakpoints, IFC label audit trails) and that a `keleusma strip` tool can remove without affecting execution semantics. B29 is independent of B28; an earlier framing tied them together through a `DataSlotAnnotation` opcode that B28 needed for runtime data-section layout, but B28's pure-runtime-refactor framing computes data-section layouts from existing wire-format metadata and no longer requires that opcode. The strippable framework remains useful for the rest of the catalogue. This entry catalogues the candidate strippable debug opcodes, names the stripper tool, and pins the wire-format invariants the design must preserve.
...
| Compiler emits `CallSite`, `SourceSpan`, `LineNumber` (the most-useful first set) | Three to five days |
| Compiler emits the remaining candidates above | Five to ten days total across the remaining opcodes |
| Runtime consumes the debug opcodes (pass-through; B29 does not feed any load-bearing behaviour) | Two to three days |
...
A reduced first commit could land the strippable-opcode mechanism plus `CallSite` (the highest-value opcode for stack-trace quality), then add `SourceSpan`, `LineNumber`, `VariableName` in follow-on commits.
...
P3d-iii is implemented, completing P3. A Fixed construct admits `+`, `-`, `*`, `/`, `%`, and unary `-`. Fixed is signed, so its outcome admissibility mirrors Word (`+`, `-`, `*` overflow and underflow, unary `-` overflow, `/` overflow and zero divisor, `%` zero divisor), but its `overflow`/`underflow` arms bind a single result like Byte and Float rather than the high and low halves, and `nan` is inadmissible. Because Q-format multiply and divide carry a fraction-bit shift that the generic `CheckedMul`/`CheckedDiv` opcodes do not encode, two new opcodes `CheckedFixedMul(u8)` and `CheckedFixedDiv(u8)` were added (wire identifiers 69 and 70, raising the instruction count from 69 to 71); the fraction-bit-free `+`, `-`, `%`, and unary `-` reuse the generic `CheckedAdd`, `CheckedSub`, `CheckedMod`, and `CheckedNeg`, which gained `Fixed` operand arms. The checked Fixed multiply and divide wrap an out-of-range result, matching the wrapping default of the other checked families, whereas the plain `FixedMul`/`FixedDiv` saturate. A zero divisor on `/` or `%` reifies as flag 3 carrying the numerator, and an unhandled zero divisor traps as a division by zero. The work landed in two commits: the dispatch-only ISA layer (opcodes, virtual-machine handlers, wire format, cost models) followed by the frontend (type-checker admissibility and the compiler emission selecting the Fixed opcodes for multiply and divide).

With P3a through P3d complete, the entire checked-arithmetic redesign is done: optional arms with wrapping defaults, the `zero_divisor` outcome, per-operand-type admissibility, and the Byte, Float, and Fixed extensions across all four numeric operand types. The remaining B35 work is the phases P4 through P9, namely the indexing construct, newtype construction, the discriminant-to-enum conversion, the native error arm, the native code generation contract, and documentation.
