...
>
> Two further debug items are carried forward beyond record emission. First, the breakpoint *runtime mechanism*: `BreakpointCandidate` is only the compiler-side candidate set; the VM does not yet hold a `breakpoint_positions` list, yield a `BreakpointHit`, or expose an arming API, and the finer function-entry and operator granularities are not yet selectable. Second, wiring the read path into the VM trap path, which needs a fault location (chunk and op index) threaded through the executor and `VmError` so a runtime trap can be mapped back to its source through the records the compiler already emits.
