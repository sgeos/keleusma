...
>
> The read path into the VM *trap* path is now wired. The VM records the op it is dispatching in a `fault_location` field (a single `Option` write per op, cleared by the `run` wrapper on success), so after a failed `call`/`resume` it names the faulting op. `Vm::fault_location() -> Option<(chunk, op)>` exposes it, and `Vm::fault_source_location() -> Option<FaultSource>` maps it back to source by decoding the faulting chunk's debug pool on demand: tier 1 returns a span-bearing record (`CallSite`, `SourceSpan`, `AssertionContext`, `BreakpointCandidate`) sitting exactly at the fault op (`exact = true`); tier 2 falls back to the nearest enclosing statement's `SourceSpan` (`exact = false`). It never fabricates a location, and the owned `FaultSource` carries the `exact` flag so a host does not over-trust a fallback. A failed debug `assert` resolves exactly through its `AssertionContext`, and a division or modulo by zero resolves exactly through the `SourceSpan` the compiler now emits at the `Div`/`Mod` operator op. The compiler also emits a `SourceSpan` for each block's tail expression, so a fault in a function whose body is one expression resolves to that expression's span rather than to nothing. A mid-statement trap at an operation that carries no operator-site span (an array index, a newtype construction) still resolves to the tightest enclosing statement or expression (`exact = false`); extending operator-site spans to those remaining partial operations would make them exact too.
>
> The three precision refinements have been addressed to the extent each is cleanly actionable; what remains in each is a deliberately bounded residual rather than a coverage gap: (1) **operator `BreakpointCandidate` granularity** — candidates emit at statement boundaries, tail expressions, trap-bearing operators (`Div`/`Mod`), and function entry; a candidate at *every* operator op is left unimplemented as high-volume and marginal. (2) **per-segment Reentrant WCET** — `wcet_whole_chunk` now returns the exact per-resume WCET (the maximum inter-yield segment cost) when yields are top-level, falling back to the sound cumulative bound only when a yield is nested in control flow; per-segment costing across a nested yield (which would require splitting a control-flow block) is the residual. (3) **per-op `SourceSpan` granularity** — operator-site spans make division and modulo faults resolve exactly and tail-expression spans cover one-expression bodies; extending operator-site spans to the remaining partial operations (array index, newtype construction) would make those exact too, and full per-op spans remain unimplemented as high-volume.
