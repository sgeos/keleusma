...
**Confirmed open gap: trait-method dispatch on any generic struct (pre-existing, not const-generic-specific).** A trait implemented for a generic struct does not resolve, and this predates and is independent of const generics. An empirical baseline established three cases. A trait method on a concrete struct works. A trait method on a **type-generic** struct, `impl Cap for Cell<T>` with a `Cell<Word>` receiver, fails in the first-pass type checker with `method cap receiver expects Cell<T>, got Cell<Word>`, because the impl's generic receiver type is not instantiated against the concrete receiver during method resolution. A trait method on a **const-generic** struct, `impl Cap for Buf<n>` with a `Buf<4>` receiver, fails after monomorphization with `type Buf__c4 has no method cap from any trait in scope`, because the monomorphizer specializes the struct to `Buf__c4` but does not specialize or re-key the impl block, so the method chunk stays registered under the generic head `Buf` while dispatch computes the head `Buf__c4`. Method dispatch keys on the receiver type head (`Trait::Head::method`), and neither generic form reconciles the head between the impl declaration and the specialized receiver. Const-generic struct and enum data otherwise work in full: fields, construction, indexing, and arithmetic. This gap is best tracked as its own item, "methods on generic structs," rather than folded into B40, because closing it requires first-pass type-checker method-receiver instantiation for generic receivers as well as impl-block specialization and dispatch-head reconciliation; it is a feature, not a const-generics residual.
