...
**Confirmed open gap: trait-method dispatch on a const-generic type.** A trait implemented for a const-generic type, `impl Cap for Buf<n>` (or `Buf<4>`, or with an `impl<const n: Word>` parameter list), does not survive specialization. After the struct specializes to `Buf__c4`, calling a method fails the re-typecheck with `type Buf__c4 has no method cap from any trait in scope`. The monomorphizer specializes the struct and rewrites its type references but does not specialize or re-key the impl block, so the trait impl remains registered under the generic name and the specialized receiver finds no impl. All three declaration syntaxes fail identically. This is a genuine bug rather than merely untested, and it is a follow-up. Trait methods on type-generic (non-const) structs are unaffected. The fix parallels the function and struct-field signature-resolution passes: an impl block over a const-generic receiver must be specialized per const value, its target type and method signatures rewritten to the specialized name, and the method registered against that name.
