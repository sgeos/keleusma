...
>
> Emitted today: `CallSite` (per call instruction), `SourceSpan` and `LineNumber` (per statement), and `VariableName` (per declared local slot) — the four kinds with a direct data source on the AST. The remaining seven kinds are defined, encodable, decodable, and (for span kinds) resolvable, but are not yet emitted because their data is not threaded to the compiler backend: `TypeAnnotation` needs a `TypeRepr` encoding and a per-op operand-stack type model; `GenericInstantiation` needs the monomorphizer to pass the generic name and type arguments to codegen; `IfcLabelAnnotation` needs the type checker's information-flow labels threaded to codegen; `WcetMarker` is a verifier-stage product computed after compilation; `AssertionContext` needs a source `assert` construct, which the language does not have; `OptimisationMarker` and `VerifierWitness` need provenance tracking that is not kept. Also carried forward: wiring the read path into the VM trap path, which needs a fault location (chunk and op index) threaded through the executor and `VmError`.
