...
>
> Two further debug items are carried forward. First, `VerifierWitness` record emission, blocked on the verifier acceptance-trace feature (above). Second, wiring the read path into the VM *trap* path: `BreakpointHit` now exposes a `(chunk, op)` position at a suspension point, which is the same shape a trap would need, but `VmError` still carries no fault location, so a runtime trap (an unhandled partial operation, or a debug `assert`) cannot yet be mapped back to its source through the records the compiler emits.
