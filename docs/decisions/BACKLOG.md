...
**Known limitations.** Indexing a struct or enum field that is itself an array (`b.items[0]`) is a pre-existing compiler limitation unrelated to const generics. Associativity is not normalized in the first-pass `Sym` comparison. Neither is a soundness gap because the re-typecheck sees only concrete literals. Impl-block methods on a const-generic type are not exercised by the current test suite; the impl parameter list carries a const-parameter vector, but method dispatch against a specialized const-generic receiver has not been verified and is a follow-up. Arity of a struct or enum const-argument turbofish is validated at the post-monomorphization re-typecheck rather than in the first pass, so a wrong count on a construction surfaces there rather than at the construction span; the function turbofish is arity-checked in the first pass.
