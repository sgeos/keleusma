...
| 7c | Default `Module::to_bytes` and `Module::from_bytes` cut over to the wire-format codec; rkyv `Module` archive removed from the execution path; rkyv-encoded `WireAuxBody` is the only remaining archive. | Pending |
