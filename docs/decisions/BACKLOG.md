...
- `docs/spec/WIRE_FORMAT.md` gains a new section documenting the optional per-chunk `debug_pool` section, its sub-pool layout including the `DebugRecord` encoding, and the candidate record catalogue when this entry is implemented.
