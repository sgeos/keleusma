...
>
> The debug items still carried forward, now that the catalogue emits in full, the `VerifierWitness` trace is per-construct and produced inline by the verify routine, its structural and Stream resource-bound coverage is closed, and the trap read path is wired: the finer **function-entry and operator** `BreakpointCandidate` granularities (above); the **resource-bound witness scope**, which is keyed to Stream chunks only (a Func or Reentrant chunk records no resource-bound obligation, faithful to the pipeline, which proves no standalone per-chunk bound for atomic-total modules — extending it would require new atomic-cost analysis, not a witness change); and the finer **per-op `SourceSpan` granularity** that would let a mid-statement or tail-expression fault resolve to the exact sub-expression rather than the enclosing statement.
