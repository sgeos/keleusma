...
>
> Beyond the structural passes, the compile pipeline emits **resource-bound** obligations (pass `resource-bounds`) on the arms where a bound is actually proven. For each Stream chunk: `wcet-per-iteration-bound-proven` when `wcet_stream_iteration` returns a finite cycle count (the count itself is carried by the adjacent `WcetMarker`), and `wcmu-per-iteration-bound-proven` when `wcmu_stream_iteration` returns a finite total. For each Func chunk: `wcet-per-chunk-bound-proven` and `wcmu-per-chunk-bound-proven`, computed by `wcet_func_chunk`/`wcmu_func_chunk` over the chunk's whole op range (the same cost walk as the Stream path, applied to the atomic-total body). The Func obligations are witness facts only: they are **not** folded into `module.wcet_cycles`/`module.wcmu_bytes`, which remain the per-iteration maximum across Stream chunks (a Stream-free module declares no header bound). All of these bounds are *per-iteration* or *per-call* and shallow with respect to calls (an `Op::Call` contributes its dispatch cycle, not the callee's body; there is no transitive WCET resolver). Admission of the program against a host's arena capacity is a separate load-time analysis (`verify_resource_bounds`) run at `Vm::new`, not at compile time, so the witness deliberately does not assert it. A chunk whose bound does not prove records no resource-bound obligation, so absence is the faithful signal. **Reentrant** chunks (from `yield` functions) receive no resource-bound obligation: their body spans `yield` boundaries, so a whole-body cost would conflate resumptions; a faithful per-segment WCET needs analysis the cost machinery does not yet provide.
>
...
>
> The read path into the VM *trap* path is now wired. The VM records the op it is dispatching in a `fault_location` field (a single `Option` write per op, cleared by the `run` wrapper on success), so after a failed `call`/`resume` it names the faulting op. `Vm::fault_location() -> Option<(chunk, op)>` exposes it, and `Vm::fault_source_location() -> Option<FaultSource>` maps it back to source by decoding the faulting chunk's debug pool on demand: tier 1 returns a span-bearing record (`CallSite`, `SourceSpan`, `AssertionContext`, `BreakpointCandidate`) sitting exactly at the fault op (`exact = true`); tier 2 falls back to the nearest enclosing statement's `SourceSpan` (`exact = false`). It never fabricates a location, and the owned `FaultSource` carries the `exact` flag so a host does not over-trust a fallback. A failed debug `assert` resolves exactly through its `AssertionContext`; a mid-statement partial-operation trap (a bare expression carries no `SourceSpan`) resolves to the enclosing statement or, for a tail expression with no enclosing statement, to nothing — a faithful reflection of the records' statement-level granularity.
>
> The debug items still carried forward, now that the catalogue emits in full, the `VerifierWitness` trace is per-construct and produced inline by the verify routine, its structural and Stream resource-bound coverage is closed, and the trap read path is wired: the finer **function-entry and operator** `BreakpointCandidate` granularities (above); the **Reentrant resource-bound witness**, the one remaining resource-bound gap now that Stream and Func chunks both emit per-iteration / per-chunk obligations: a Reentrant chunk's body spans `yield` boundaries, so a faithful per-segment WCET needs analysis (cost between consecutive yields, as the Stream path isolates one Stream-to-Reset iteration) that the cost machinery does not yet provide; and the finer **per-op `SourceSpan` granularity** that would let a mid-statement or tail-expression fault resolve to the exact sub-expression rather than the enclosing statement.
