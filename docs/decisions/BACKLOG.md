...
P3d-ii Float remains: a Float construct uses `ok`, `overflow` for positive infinity, `underflow` for negative infinity, and `nan`, with no `(high, low)` decomposition and no trap, since the Institute of Electrical and Electronics Engineers 754 operations are total. Then the phases P4 through P9.
