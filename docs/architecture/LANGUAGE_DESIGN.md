...
V0.2.0 also admits *negative* labels at function parameter and return type positions through the `!Label` prefix: `T@!Label` and `T@{!N1, !N2}`. A negative-label position is a boundary clause: a value flowing across a parameter, return, or yield boundary may not carry any of the listed labels. The positive-label upper-bound rule is relaxed at boundaries with negative labels so any other label is admissible. Mixed positive-and-negative sets are rejected at parse time. Negative labels do not propagate as a labelled type through the lattice; the underlying type of a negative-labelled parameter is just the inner type. Value-side negative labels (a product-lattice extension that tracks "this value lacks label X" through arithmetic and branching) are deferred to a future iteration; see `R43` in [`docs/decisions/RESOLVED.md`](../decisions/RESOLVED.md) for the design and `B21` in [`docs/decisions/BACKLOG.md`](../decisions/BACKLOG.md) for the value-side extension on the shelf.
