...
Private data is script-initialized at load, the assembler `.data`-section model, and is invisible to the host. Each scalar private slot (`Word`, `Byte`, `bool`, `Fixed`, `Float`) initializes to its declared `= literal` initializer or, absent one, the type's zero, baked by the compiler into the module's `DataLayout::private_init` table and written into the persistent region when the VM is constructed. The values persist across RESET and are not re-applied. A read of a scalar private slot before any write is therefore well-defined and observes the initial value rather than faulting. Scalar private fields are the only ones that admit a `= literal` initializer; a composite private slot (struct, tuple, enum, or array of composites) initializes to `Unit` and retains the write-before-read contract, and `shared` fields never carry initializers because the host supplies their values. In contrast to a private slot, a shared slot is host-initialized before execution begins.
...
Keleusma is a total functional language, yet several operations are mathematically partial, meaning undefined on some inputs. These are integer and fixed-point division and modulo on a zero divisor, array indexing out of bounds, refinement-newtype construction whose predicate fails, the discriminant-to-enum conversion on an invalid discriminant, and fallible native function failures. The B35 design gives every such operation a defined contract and an opt-in handling mechanism, so a program can be made total at the source level rather than relying on a runtime trap. This replaces the narrower V0.2.0 numeric-overflow construct, which required a covering arm for every outcome and admitted only `Word` operands.

The handling mechanism is a family of constructs, each a match block over a fallible operation distinguished by a fixed vocabulary of outcome-arm keywords. The six members are checked arithmetic over `Word`, `Byte`, `Float`, and `Fixed<N>` operands, array indexing, newtype construction, the discriminant-to-enum conversion, and the native call. The arms are optional unless noted, and omitting an admissible arm invokes a defined default. An arm whose outcome cannot arise for the operation is a compile error, and the type checker enforces both the vocabulary and the per-construct coverage rule. The formal grammar of each construct is in [GRAMMAR.md](../spec/GRAMMAR.md).

Checked arithmetic guards a single operation. The form `expr { ok(v) => ..., overflow(...) => ..., underflow(...) => ... }` matches the in-range result, a positive overflow, and a negative overflow. On the signed `Word` type the runtime computes the true result in the next-larger signed integer and pushes the high and low halves so the arms can destructure them, the load-bearing mechanism for big-number arithmetic. On `Byte`, `Float`, and `Fixed<N>` the arms bind a single result. For division and modulo a `zero_divisor` arm binds the numerator. The `overflow` and `underflow` arms are optional and default to two's-complement wrapping, so an `ok`-only construct is equivalent to the bare wrapping operation. The `saturate_max` and `saturate_min` keywords inside arm bodies denote context-determined saturation values, resolved at the operand type: the `Word`, `Byte`, `Float`, or `Fixed<N>` bound, or, when the expected type is a refined newtype with a `with saturate_max` or `with saturate_min` clause, a constructor call against the declared literal.

The other five constructs follow the same arm-block shape. Array indexing admits `ok` and `invalid_index`. Newtype construction admits `ok` and `invalid_newtype`. The discriminant-to-enum conversion `discriminant as Enum { ... }` admits `ok`, `payload_discriminant`, and `invalid_discriminant`, and is the reverse of the enum-to-`Word` cast. The native call admits `ok` and `error`, where `error` binds the `Word` error code a fallible native reports. The host produces that code through the `KeleusmaError` derive, which maps a fieldless enum's variants to their discriminants.

The contract has two backends. The verifying virtual machine traps recoverably on any unhandled partial operation, returning a structured error the host categorizes. A future native backend produces a defined, non-crashing value, using the hardware result where the hardware does not fault and an inserted guard where it would. The two backends diverge only on unhandled partial operations, so a fully handled program produces identical results on both. The complete contract, the per-operation trap variants, the native default values, and the per-target hardware basis are specified in [RUNTIME_FAULTS.md](../spec/RUNTIME_FAULTS.md). The native lowering is deferred to V0.4.0.
...
V0.2.x admits *negative* labels at boundary positions through the `!Label` prefix: `T@!Label` and `T@{!N1, !N2}`. A negative-label position is a boundary clause: a value flowing across the boundary may not carry any of the listed labels. Three boundary-position categories are recognised: function parameter and return types (the call-site, return, and yield boundaries), `shared` data field types (the host-script boundary; the host writes through `Vm::set_data` and reads through `Vm::get_data`, and the script writes through field assignment), and `private` data field types (the yield-resume boundary; values written before a yield are read after a resume). The positive-label upper-bound rule is relaxed at boundaries with negative labels so any other label is admissible. Mixed positive-and-negative sets are rejected at parse time. Negative labels do not propagate as a labelled type through the lattice; the underlying type of a negative-labelled parameter or data field read is just the inner type. The negative wrapper is admissible only at top-level positions; nested positions inside tuples, arrays, or options reject it. Value-side negative labels (a product-lattice extension that tracks "this value lacks label X" through arithmetic and branching) are deferred to a future iteration; see `R43` in [`docs/decisions/RESOLVED.md`](../decisions/RESOLVED.md) for the design and `B21` in [`docs/decisions/BACKLOG.md`](../decisions/BACKLOG.md) for the value-side extension on the shelf.
