...
| `.rodata` | `Module::chunks[i].constants: Vec<ConstValue>` and `Module::chunks[i].struct_templates: Vec<StructTemplate>` in `src/bytecode.rs`. Enum metadata for the boxed form is carried by a per-chunk struct template naming the variant; the flat form's discriminant is supplied as an operand-stack constant that `Op::NewComposite` packs as the leading field. | `compile_with_target` writes the constant pool while lowering each function body. Per-chunk struct templates accumulate during lowering as the compiler encounters struct definitions. The native function name table at `Module::native_names` is also `.rodata` content because it is read-only after compile. | `Vm::chunk_const` lifts a `ConstValue` to a runtime `Value` through `Value::from_const_archived`. `Op::Const`, `Op::NewComposite`, `Op::CallVerifiedNative`, and `Op::CallExternalNative` index these tables at dispatch time. |
| `.data` | Shared slots live in a host-owned `&mut [u8]` buffer borrowed for the duration of each call (B28 item 2); private slots live in the arena's persistent region. The layout schema is `Module::data_layout: Option<DataLayout>`. | The host sizes the shared buffer with `Vm::shared_data_bytes()` and lends it at each `Vm::call_with_shared`/`Vm::resume_with_shared`. Private slots are zero-initialized in the arena at construction. `Vm::replace_module` takes a fresh PRIVATE-only `initial_data`; the shared buffer is the host's and persists across the swap in the host's own storage, and the schema may change. | `Op::GetData(slot)` and `Op::SetData(slot)` dispatch a shared slot to the borrowed buffer by byte offset and a private slot to the arena. Between calls the host reads and writes scalar shared fields through `Vm::get_shared` and `Vm::set_shared`. |
...
Memory bookkeeping. The arena's bottom-region budget covers the `.bss` operand-stack and call-frame peak. The arena's top-region budget covers `.bss` `KString` allocations from arena-aware natives. The two are checked together against the arena capacity by `verify::module_wcmu` at `Vm::new` and `Vm::replace_module`. The shared `.data` is the host-owned buffer borrowed at each call, sized by the host to `Module::shared_data_bytes` and outside the arena's WCMU budget. The private `.data` lives in the arena's persistent region, sized from the `DataLayout` and checked against the arena's persistent capacity at construction and hot swap. The `.text` region's storage cost is the program's total op count multiplied by `size_of::<Op>()` plus the rkyv-archived bytecode buffer; both live in the global heap.
...
Concurrency is single-ownership. The script holds exclusive access to the segment while executing. Ownership returns to the host at YIELD and at RESET. Concurrent access from another host thread during script execution is unspecified. In the Rust host environment, the borrow checker enforces single ownership at compile time. The host lends the shared segment as a `&mut [u8]` borrowed only for the dynamic extent of one `Vm::call_with_shared` or `Vm::resume_with_shared`, so the host cannot alias or mutate it during script execution, and the VM retains no pointer into it past the call's return. Between calls the host owns the buffer outright and may read, write, swap, or drop it.
...
The shared data segment is a host-owned external buffer of a fixed flat layout, lent to the VM by `&mut [u8]` at each call and read and written in place by byte offset. This realizes the host-owned, borrowed, swap-or-mutate-at-resume contract directly. An earlier build kept shared data in a VM-owned `Vec<Value>` slot vector accessed through `set_data`/`get_data`, chosen at the time to avoid raw-pointer manipulation; B28 item 2 reversed that choice deliberately to give the host a flat byte buffer it owns outright and to free the runtime of the owned-`Value` body that blocked the flat-composite collapse. The raw-pointer handling the borrowed buffer needs is confined to one small module, `src/shared_buf.rs`, whose `SharedBuf` captures the borrow for one call and whose only two unsafe-bearing methods reconstruct the slice under an invariant discharged at the call boundary; the rest of the runtime works with safe slices. The host is free to back its application-level state in any Rust struct it prefers and to marshal between that struct and the flat buffer.

The Vm public API for the data segment consists of the following methods.

| Method | Use |
|---|---|
| `Vm::new(module)` | Construct the VM. Private slots are zero-initialized in the arena; shared data is the host's buffer, supplied per call. |
| `shared_data_bytes()` (and free `shared_data_bytes_for(module)`) | Flat byte length the host allocates for the shared buffer. |
| `call_with_shared(&mut [u8], args)` / `resume_with_shared(&mut [u8], value)` | Run an entry point with the shared buffer lent for the call. The buffer length must equal `shared_data_bytes`; the plain `call`/`resume` forward an empty slice and suit modules with no shared data. |
| `get_shared(&[u8], slot)` / `set_shared(&mut [u8], slot, value)` | Read or write a scalar shared field in a host buffer between calls. |
| `replace_module(new_module, initial_data)` | Hot swap the code atomically. Verifies the new module. `initial_data` supplies only the new module's PRIVATE slots; shared data is the host's buffer, which carries across the swap in the host's own storage. Clears frames and stack so the next `call_with_shared` starts the new module's entry point. Suitable for both forward update and rollback. |

Schema mismatch detection is by size check plus host attestation. The size check compares the supplied private-data vector length against the declared private-slot count of the new module, and the shared-buffer length against `shared_data_bytes` at each call. Dialogue type compatibility between modules across a swap is the host's responsibility because dialogue types are erased at the bytecode level. Schema hash comparison and structural type checking against a schema descriptor are deferred to a later phase.
...
| Bytes | Field | Notes |
|-------|-------|-------|
| 0 | `opcode_id` (u8) | The dispatch index. The low 7 bits select among the live opcodes; opcode values 0-33 and 38-69 are valid (ids 34-37 are retired by the B28 consolidation and reserved), and values 70-127 are reserved. The high bit is a **parity bit** over bytes 0-3 of the record, supporting per-record integrity verification at decode time. |
...
| Compound `(u16, u16)` | Bytes 1-3 hold a 24-bit little-endian index into the operand pool (up to `1 << 24` entries). Carried by `GetDataIndexed`, `SetDataIndexed`, `IsEnum`. |
| `NewComposite` | Flat form: bytes 1-2 hold the flat byte size; byte 3 packs the composite kind (high 2 bits) and the operand-stack pop count, 0-62 (low 6 bits). Boxed form, or a flat field count above 62: byte 3 holds the kind and the `0x3F` sentinel, and bytes 1-2 hold a 16-bit index into a `(u16, u16, u8)` operand-pool entry carrying `(count, byte_size-or-meta, boxed_flag)`. |

62 of the 66 live opcodes always carry their operand inline in the 4-byte record. Three opcodes — `GetDataIndexed`, `SetDataIndexed`, and `IsEnum` (all `(u16, u16)`) — always reference the operand pool because their payload exceeds three bytes. `NewComposite` carries its operand inline in the common small flat form and references a `(u16, u16, u8)` operand-pool entry only for the boxed form or a flat field count above 62.
...
The recovery model is consistent with the hot-swap design (R26, R27). Both clear volatile state while letting the host control data continuity. Private data in the arena is preserved across recovery; shared data is the host-owned buffer the host controls directly, so the host resets shared state by zeroing or replacing its own buffer. To reset private data, the host uses `Vm::replace_module` to swap to a new code image with new private initial data.
