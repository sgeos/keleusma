...
fn the_self_hosted_share_of_the_corpus_is_the_one_recorded() {
    ...
    assert!(
        ...
        "the self-hosted path produces {share}% of the corpus's region bytes ({covered} of \
         {total}). It was 99% when `ENUM_VARIANTS` landed, 98% when `DATA_SLOTS` did, 93% at \
         `SHAPES` and `SIGNATURES`, and 81% before them; a fall means a region stopped being \
         routed"
    );
    assert!(
        share < 100,
...
///
/// "The self-hosted path produces 99% of the corpus's region bytes" is true and
/// invites a stronger reading than it supports. **`DATA_SLOTS` and
/// `ENUM_VARIANTS` took it from 81 to 99 on 2026-09-11 and raised the computed
/// share not at all**: the stage supplies each region's NAME from its own
/// interner and the host decides every other field, which is the `CHUNKS`
/// standing, not the `NAMES` one. The handoff's provenance table
