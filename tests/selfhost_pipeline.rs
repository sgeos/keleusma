...
// declaration order: len (1 slot) then bytes (196608) precede it.
const LEX_ISTART: usize = 1 + 196608;
const LEX_ILEN: usize = 1 + 196608 + 1280;
const LEX_ICOUNT: usize = 1 + 196608 + 1280 + 1280;
...
fn lex_full(src: &str) -> (Vec<(i64, i64)>, Vec<String>) {
    ...
    assert!(
        bytes.len() <= 196608,
        "source exceeds the lexer's 196608-byte cap"
    );
